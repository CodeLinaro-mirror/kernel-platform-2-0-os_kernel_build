...
usage: OUT_DIR=/path/to/out/branch/ python buildinfo.py
"""
...
def make_abs(s):
  if s[2] != os.sep:
    return "-I" + os.path.join(os.environ["OUT_DIR"], s[2:])
  return s
...
def write_out(db):
  ...
  with open(path, "w") as fp:
    fp.write(json.dumps(db, indent=4))
